...
const AudioContext = () => {
  ...
  throw new Error('Missing audioFiles8');
};



const _soundPlayer_preloadCache = {};
const _soundPlayer_audioCtx = AudioContext();
...
function Mach1SoundPlayer() {
  ...
  function preload(path, i) {
    ...
    fetch(path, {
      ...
      cache: 'force-cache'
    }).then((res) => res.arrayBuffer()).then((blob) => {
      if (_soundPlayer_preloadCache[path]) {
        return Promise.resolve(_soundPlayer_preloadCache[path]);
      }
      return _soundPlayer_audioCtx.decodeAudioData(blob, (data) => Promise.resolve(data));
    }).then((aBuffer) => {
  ...
  this.getBuffer = function () {
    const arr = initArray(SOUND_COUNT / 2);
